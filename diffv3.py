...
class ThermalDiff:
    ...
    This is the model for coupled thermal diffusion into an infinite space with a constant dispersion from a sphere.
    ...
    ***How to use code***
    A class instance should be called to initialise the model, then only run_model needs to be called to output the concentration profiles.     
    
    ***Variables***
    x1 and x2 should not be changed, they are used to find roots for the thermal diffusion equation, which is an arctan function. The root finding functions in python packages took ages. 
    
    Bi is the Biot Number see https://en.wikipedia.org/wiki/Biot_number
    
    n_search = the number of points to search for roots, should be ~Bi*1000
    
    R_steps = the number of distance steps or the spatial resolution for the model
    
    R = the radius of the sphere that is being simulated. meters
        
    Ti = initial uniform Temperature profile
    
    Tout = external temperature, diffusion is moderated by the Biot number
    
    alpha = thermal diffusion coeffient
        
    t_steps = number of temperature steps, can be set to "auto" - for repeatable results should be set to satisfy the stability criteria : dt = (0.5*((R/R_steps)**2))/(D)
    
    t_max = maximum time to run simulation for, seconds
    
    plot = number of intermediat ssteps to plot, if set as num(t_steps) code will run very slowly as each step will be appended to numpy array
    
    returns 
    
    plots are produced along with the thermal profiles
    """ 
...
class StepDiffusion:
    ...
    This is the model for diffusion with constant temperature across the model for each time step.
    ...
    ***Variables***
    ...
    C0 = uniform concentration of halogen through the sphere
    
    Cout = constant concentration of halogen in the atmosphere, note the partition coefficient is implicitally assumed to be 1. 
    
    Tc = temperature (C) 
    
    delT = the temperature drop accross the whole time step, lineally interpolated for each time step
    
    element = F, Cl, Br, I - code can be modifed to work with other elements if the diffusion coefficients are known.
    ...
    legend = bool, if tru graphs will have legends
    
    plotDetectionLimit = bool, if true detection limit for SIMS will be plotted (can be modified above)
    
    sphericFactor = bool, to approximate a sphere a factor is used to modify the mass transfer between shells. If a 1d object should be simulated this should be set to False.  
    
    
    returns 
    
    plots are produced along with the concentration profiles and a spheric factor array    
    """     
...
class CoupledModel:
    ...
    This is the model for coupled thermal and material diffusion from a finite sperical object into an infinite space with a constant dispersion.
